...
const Wallet: React.FC<{}> = () => {
    ...
    const MoneyCard = (currency: ICoinsKeys, value: number, index: number): JSX.Element => {
            ...
            ...user?.trades // ALL TRADES MADE BY THIS USER
                ...
                .map(trade => {                                  // LET US WORK WITH CURRENCY "X"
                    ...
                    if (trade.buy && trade.from === currency) {    // when we sell an amount of Y to get 1 X
                        ...
                        return ({ money: trade.value, time: trade.when }) // we should increase this amount of X 
        ...
        return (
            <div className={"wallet-card" + (currencyIndex === index ? " selected" : "")} key={index} style={{ backgroundColor: colors[index], "--selected-color": colors[index] } as MyCustomCSS}
            ...
            {user?.wallet ?
                <>
                    <div className={"wallet-cards"}>
                        ...
                        <div className={`wallet-activities-card ${currencyIndex === -1 && 'selected'}`} onClick={() => setCurrencyIndex(-1)}>
                            <span>Activities</span>
                        </div>
                    </div>
                    {currencyIndex < 0 && (user?.trades.length || user.purchases.length ?
                        <div style={{ margin: "20px 5%" }}><PreviousTrades purchases={user?.purchases} data={user?.trades as IMyTrade[]} /></div> :
                        <Typography className="App"><br />You have no trades yet.<br /> <a href="/trade">Trade now</a></Typography>)}
                    {currencyIndex >= 0 &&
                        <>
                            ...
                            {user.trades.length || user.purchases.length ?
                                ...
                                : <b style={{ margin: '20px 5%', display: 'flex' }}>No {currencyIndex === 0 ? "US$" : "£"} activity yet</b>}
                        </>
                    }
                    <Box>
