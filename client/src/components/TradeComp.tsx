...
const TradeComp:React.FC<{}> = () =>{
  ...
  return (
    <div>
      ...
      <Box className={classes.mBottom}><Button title="Switch currencies" variant="outlined"
       onClick={()=>
        setOrder(default_order.from === "GBP" ?
         {from:'USD', to:"GBP"} :
          {from:'GBP', to:"USD"} )}>{default_order.from} to {default_order.to}</Button>
          </Box>
      <Box>  
        <ResponsiveContainer width="60%" height={300} className={classes.chart}>
          <ComposedChart data={currentTr} >
            ...
            <Line name="Buying price" type="monotone" dataKey="ask" strokeWidth={3} stroke={pink[400]} animationDuration={500} />
            <Line name="Selling price" type="monotone" dataKey="bid" strokeWidth={3} stroke={green[500]} animationDuration={500} />
          </ComposedChart >
        </ResponsiveContainer>
      </Box>
